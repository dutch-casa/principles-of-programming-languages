...
(define extend-env
 (lambda (env var val)
  (cons (list 'extend-env var val) env)))

  ;; update-env : environment, variable, value -> environment
  (define update-env
    (lambda (env var val)
      (cond
        ((eqv? (car env) 'empty-env) (report-no-binding-found var))
        ((eqv? (car env) 'extend-env) (let ((saved-var (cadr env))
                                            (saved-val (caddr env))
                                            (saved-env (caddr env)))
                                        (if (eqv? var saved-var)
                                            (cons (list 'extend-env var val) saved-env)
                                            (cons (list 'extend-env saved-var saved-val) (update-env saved-env var val)))))
...
(define in-env
  (lambda (env var)
    (cond
      ...
      ((eqv? (car env) 'extend-env) (if (eqv? var (cadr env))
                                       #t
                                       (in-env (cdr env) var)))
      (else (report-invalid-env env)))))
